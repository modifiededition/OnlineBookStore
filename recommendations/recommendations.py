...
class RecommendationService(recommendations_pb2_grpc.RecommendationsServicer):
    ...
    def Recommend(self, request, context):
        if request.category not in books_by_category:
            context.abort(grpc.StatusCode.NOT_FOUND, "Category Not Found in the database.")
        
        book_for_category = books_by_category[request.category]
        num_results = min(request.max_results, len(book_for_category))
        books_to_recommend = random.sample(
            book_for_category, num_results
        )

        return RecommendationResponse(recommendation = books_to_recommend)
